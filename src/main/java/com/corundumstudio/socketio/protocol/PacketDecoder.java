...
public class PacketDecoder {
    ...
    private boolean isStringPacket(ByteBuf content) {
        byte firstByte = content.getByte(content.readerIndex());
        return firstByte == 0x0;
    }

    private boolean isJsonPacket(ByteBuf content) {
        byte firstByte = content.getByte(content.readerIndex());
    ...
    private boolean hasLengthHeader(ByteBuf buffer) {
        int max = Math.min(buffer.readableBytes(), 10);
        for (int i = 0; i < max; i++) {
    ...
    public Packet decodePackets(ByteBuf buffer, ClientHead client) throws IOException {
        if (isStringPacket(buffer)) {
            logger.info("## packet content is string");
            // TODO refactor
        ...
        } else if (isJsonPacket(buffer)) {
            // TODO
            return decodeJson(client, buffer);
        } else if (hasLengthHeader(buffer)) {
            logger.info("## packet hasLengthHeader is true");
            // TODO refactor
    ...
    private Packet decodeJson(ClientHead head, ByteBuf frame) {
        String json = frame.toString(CharsetUtil.UTF_8);
        PacketProtocol packetProtocol = ObjectMapperUtils.fromJSON(json, PacketProtocol.class);
        Packet packet = new Packet(packetProtocol.getType());
        packet.setSubType(packetProtocol.getSubType());
        packet.setNsp(packetProtocol.getNsp());

        parseBodyForJson(packet, packetProtocol);
        return packet;
    }

    private void parseBodyForJson(Packet packet, PacketProtocol packetProtocol) {
        //packet.setName();
        RawCommand rawCommand = ObjectMapperUtils.fromJSON(ObjectMapperUtils.toJSON(packetProtocol.getData()), RawCommand.class);
        packet.setData(Arrays.asList(rawCommand));
    }
