...
public class InPacketHandler extends SimpleChannelInboundHandler<PacketsMessage> {
    ...
    protected void channelRead0(io.netty.channel.ChannelHandlerContext ctx, PacketsMessage message)
        ...
        if (log.isDebugEnabled()) {
            log.debug("## InPacketHandler message:{},sessionId:{},transport:{}", content.toString(CharsetUtil.UTF_8), client.getSessionId(), transport);
        }
        ...
        if (content.isReadable()) {
            try {
                ...
                // TODO
                packet.setNsp(Namespace.DEFAULT_NAME);
                packet.setName(EventConstants.EVENT_KEY);
