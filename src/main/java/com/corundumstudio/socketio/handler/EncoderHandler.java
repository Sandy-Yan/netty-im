...
public class EncoderHandler extends ChannelOutboundHandlerAdapter {
    ...
    private void handleWebsocket(final OutPacketMessage msg, ChannelHandlerContext ctx, ChannelPromise promise) throws IOException {
        log.info("## handle Websocket begin");
        while (true) {
            ...
            encoder.encodePacket(packet, out);

            WebSocketFrame res = new TextWebSocketFrame(out);
            if (log.isDebugEnabled()) {
                log.debug("## out message: {} sessionId: {}", out.toString(CharsetUtil.UTF_8), msg.getSessionId());
            }
            log.info("## out.isReadable:{}", out.isReadable());
            if (out.isReadable()) {
                log.info("## promise.isDone:{}", promise.isDone());
                if (!promise.isDone()) {
            ...
            log.info("## process packet attachments, attachments:{}", packet.getAttachments());
            for (ByteBuf buf : packet.getAttachments()) {
                /*ByteBuf outBuf = encoder.allocateBuffer(ctx.alloc());
                outBuf.writeByte(4);
                outBuf.writeBytes(buf);*/
                ByteBuf outBuf = encoder.encodeAttachment(packet, buf, ctx.alloc());
                if (log.isDebugEnabled()) {
                    log.debug("## out attachments: {} sessionId: {}", ByteBufUtil.hexDump(outBuf), msg.getSessionId());
                }
